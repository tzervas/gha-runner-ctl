...
| — | `GHA_POOL_SPAWN_GRACE_SECS` | u64 | `90` | Grace window (seconds since spawn) protecting a freshly-spawned, never-dispatched worker from `post-job-exit` reclaim; `busy=0` inside this window is never read as "job finished" (issue #127) |
