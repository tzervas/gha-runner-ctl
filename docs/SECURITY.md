...
| Micro-agent image | Binary + CA only | Ubuntu-minimal stripped; UID 65532 nonroot; no shell, no sudo, no Podman socket — cannot spawn containers if compromised |
...
| Micro-agent container | Ubuntu-minimal stripped `nonroot` (65532) | none (no socket, no CLI) | none |
